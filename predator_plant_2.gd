...
func _ready() -> void:
	...
	animated_sprite.animation = "predator_plant_2"
	...
	if is_player_detected and is_instance_valid(player):
		...
		if abs(dir_x) > 2.0:
			if dir_x > 0:
				...
				# Player di KIRI → posisi normal (menghadap kanan)
...
func _on_body_entered(body: Node2D) -> void:
	...
	if body.name == "player":
		...
		animated_sprite.play("predator_plant_2")
